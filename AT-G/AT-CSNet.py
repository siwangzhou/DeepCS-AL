...
model_dir_G = "./AT_CSNet_G_cifar10/%s/model_G" % sample_rate
model_dir_D = "./AT_CSNet_G_cifar10/%s/model_D" % sample_rate
output_file_name = "./AT_CSNet_G_cifar10/%s/Log_output_AT_CSNet_G_cifar10_%s.txt" % (sample_rate, sample_rate)
mat_save_path = "./AT_CSNet_G_cifar10/%s/predict.mat" % sample_rate

def read_validation_batch():
    testDataset_path = "../DeepCS-AL/datasets/imagenet/testData/test.mat"
...
def Generator(X):
    with tf.variable_scope("CSNet"):
        ...
        with tf.variable_scope("sampling"):
            W_conv1 = weight_variable('W_conv1', [B, B, 1, nb])  #测量矩阵
            h_conv1 = conv2d_VALID(X, W_conv1, stride=[1, B, B, 1])

        with tf.variable_scope("initial_reconstruct"):
            W_conv2 = weight_variable('W_conv2', [1, 1, nb, B * B])
            h_conv2 = conv2d_VALID(h_conv1, W_conv2, stride=[1, 1, 1, 1])
    ...
    return W_conv1, h_concat, h_conv12
...
def build_GAN(x_real, dropout_rate, is_training):
    W_conv1, h_concat, fake_images = Generator(x_real)
...
def optimizer(D_Loss, G_Loss, lr_g, lr_d):
    ...
    with tf.control_dependencies(update_ops):
        ...
        G_vars = [var for var in all_vars if var.name.startswith('CSNet')]

        d_train_opt = tf.train.AdamOptimizer(lr_d, beta1=0.5).minimize(D_Loss, var_list=D_vars)
        g_train_opt = tf.train.AdamOptimizer(lr_g, beta1=0.9, beta2=0.999).minimize(G_Loss, var_list=G_vars)  #beta1
...
def train_model():
    ...
    all_vars = tf.global_variables()
    G_vars = tf.trainable_variables(scope="CSNet")
    D_vars = [var for var in all_vars if var.name.startswith('Discriminator')]
    ...
    with tf.Session() as sess:
        if model == "train":
            ...
            saver_G.restore(sess, "../models/CSNet/%s/model/Model_99.ckpt" % sample_rate)
            ...
            ps, ssim_val = [], []
            for i in range((int)(np.shape(val_im)[0] / batch_size)):
                test_batch_x = val_im[i * batch_size: (i + 1) * batch_size]
                test_batch_y = val_cls[i * batch_size: (i + 1) * batch_size]
                test_feed_dict = {X: test_batch_x, Y: test_batch_y, keep_prob: 1.0, is_training: False}
